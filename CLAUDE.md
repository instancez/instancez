...
- `ULTRABASE_AUTH_DATABASE_URL` → `authenticator` login that `INHERIT`s the `anon` / `authenticated` / `service_role` roles. The request path acquires a connection, runs `SET LOCAL ROLE <role>` per transaction based on the validated JWT, executes the query under RLS, and resets on release. Lives behind `domain.RequestDB`. See `internal/adapter/postgres/context.go` and `pool.go`.
