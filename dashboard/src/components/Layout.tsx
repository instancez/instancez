...
import { SaveBarDockContext } from "./saveBarDock";
    ...
    <Box h="100dvh" bg="bg" display="flex" flexDirection="column" overflow="hidden">
      ...
      <HStack flex="1" minH="0" gap="2" px="2" pb="2" align="stretch">
        ...
        <Box
          ...
          boxShadow="xs"
        >
          {/* The scroll region fills the card; the docked SaveBar slot below it
              stays pinned to the card's bottom edge, so the bar reserves its own
              row instead of floating over the content. */}
          <SaveBarDockContext.Provider value={dockNode}>
            <Box flex="1" minH="0" overflowY="auto">
              {/* Depth 1: page content sits on the surface card, so Panels
                  inside it render as gray insets, and their children flip
                  back to surface, so every box contrasts with its parent. */}
            ...
            <div ref={setDockNode} style={{ flexShrink: 0 }} />
          </SaveBarDockContext.Provider>
        </Box>
      </HStack>
