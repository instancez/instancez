...
export function Login({ onSuccess }: LoginProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    setError("");
    const valid = await validateSecretKey(key.trim());
    if (valid) {
      sessionStorage.setItem("instancez_secret_key", key.trim());
      onSuccess();
    } else {
      setError("Invalid secret key. Check INSTANCEZ_SECRET_KEY.");
    }
    setLoading(false);
  }
          ...
          <VStack gap="8" mb="8" textAlign="center">
            ...
            <Box>
              ...
              <Text mt="1.5" fontSize="sm" color="fg.muted">
                Enter your secret key to open the dashboard
              </Text>
            </Box>
          </VStack>

          <VStack as="form" onSubmit={handleSubmit} gap="5" align="stretch">
            <Field label="Secret Key" htmlFor="secret-key" align="center">
              <Box position="relative" maxW="xs" mx="auto" w="full">
                <Box
                  as={KeyRound}
                  boxSize="4"
                  position="absolute"
                ...
                <Input
                  ...
                  placeholder="INSTANCEZ_SECRET_KEY"
                  autoFocus
                  fontFamily="mono"
                  pl="9"
                  size="sm"
