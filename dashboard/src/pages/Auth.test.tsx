...
describe("AuthPage", () => {
  ...
  it("renders literal OAuth settings as plain editable inputs, creds first", () => {
    ...
    // Redirect URL is fixed and derived, shown read-only for copying.
    const redirect = screen.getByLabelText("Redirect URL");
    expect(redirect).toHaveValue(CALLBACK_BASE + "google");
  ...
  it("enabling a provider stages the secret as ${VAR} and settings as literals", async () => {
    ...
    // The derived callback shows immediately, even before anything is saved.
    expect(screen.getByLabelText("Redirect URL")).toHaveValue(CALLBACK_BASE + "google");
  });
  ...
  it("saves an enabled provider under auth.oauth, not as a top-level key", async () => {
    ...
    // redirect_url is fixed/derived by the engine, never sent from the dashboard.
    expect(saved.oauth.google).not.toHaveProperty("redirect_url");
