...
describe("ProvidersPage", () => {
  ...
  it("shows 'not set' (read-only) when getEnvVars returns set=false", async () => {
  ...
  it("shows a masked tail when getEnvVars returns set=true", async () => {
    vi.mocked(api.getEnvVars).mockResolvedValue({
      vars: { INSTANCEZ_RESEND_API_KEY: { set: true, tail: "wxyz" } },
    });
    const config: Config = {
      ...baseConfig,
      providers: {
  ...
  it("optimistically shows the masked tail after a save, without waiting for the env to reload", async () => {
    // The process env updates asynchronously after the write (dev file watcher /
    // platform env sync), so getEnvVars keeps reporting unset right after saving.
    // The just-written secret must still show its masked tail immediately.
    vi.mocked(api.getEnvVars).mockResolvedValue({ vars: {} }); // always stale/unset
    const config: Config = {
      ...baseConfig,
      providers: {
    ...
    renderProvidersWithCtx(config, true);
    fireEvent.change(screen.getByLabelText("INSTANCEZ_RESEND_API_KEY"), {
      target: { value: "re_secret_value_abcd" },
    });
    fireEvent.click(screen.getByRole("button", { name: /save/i }));
