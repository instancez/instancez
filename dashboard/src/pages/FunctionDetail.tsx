...
import type { CodeFunction, ValidationError } from "../lib/types";
...
export function FunctionDetail() {
  ...
  const isNew = !name;
  const newState = location.state as
    | { functionName?: string; seed?: CodeFunction; code?: string }
    | null;
  const { config, save, saving, saveErrors, refresh } = useConfig();
  ...
  // New-mode seeds once from the router-state seed. It must NOT re-fire on
  // config changes: a failed code write (see handleSave) leaves us in new
  // mode after the config write already succeeded and refreshed `config`,
  // and re-seeding here would silently revert the user's metadata edits.
  useEffect(() => {
    if (isNew && newState?.seed) setFn(structuredClone(newState.seed));
  }, [isNew, newState]);
  ...
  async function handleSave() {
    const effectiveName = isNew ? newState?.functionName : name;
    if (!config || !fn || !effectiveName) return;
    setFileError(null);
    setCodeError(null);

    // For an existing function, a changed file path must already exist on disk.
    // A new function has no saved file yet and writes its code just below, so
    ...
    const updated = {
      ...config,
      functions: { ...(config.functions || {}), [effectiveName]: fn },
    };

    if (isNew) {
      // One atomic create: config + code in a single request → exactly one build.
      // The checksum travels via the backend's If-Match, not the config body.
      setCreateErrors([]);
      setCreating(true);
      try {
        const { _checksum: _omit, ...configBody } = updated;
        await backend.createFunction(effectiveName, configBody, code ?? "", config._checksum ?? "");
      } catch (e: any) {
        const errs = e?.body?.errors;
        if (Array.isArray(errs) && errs.length > 0) {
          setCreateErrors(errs);
        } else if (e?.body?.error === "conflict" || e?.status === 409) {
          setCodeError(`Could not create "${effectiveName}": the draft changed since you started. Reload and try again.`);
        } else {
          setCodeError(`Could not create function "${effectiveName}": ${e.message || "save failed"}`);
        }
        return;
      } finally {
        setCreating(false);
      }
      // Bring the shared context up to date before leaving: the Functions
      // list and this page's own checksum both read from it, so a stale
      // context would hide the new function and leave a spurious dirty state.
      try {
        await refresh();
      } catch {
        // A failed refresh shouldn't strand the user on a function that
        // was already created successfully; the next navigation will refetch.
      }
      navigate(`../${effectiveName}`, { relative: "path", replace: true });
      return;
    }
  ...
  // In new mode the function is always dirty (nothing saved yet).
  const dirty = isNew || !jsonEqual(fn, (config.functions || {})[name ?? ""] ?? null);

  const envEntries = Object.entries(fn.env || {});

  return (
    <Box pb="20">
      <DetailToolbar backLabel="Code Functions" onDelete={!isNew ? deleteFunction : undefined} />
      ...
      {canWriteConfig && (
        <SaveBar
          ...
          errors={[
            ...saveErrors,
            ...createErrors,
            ...(fileError ? [{ path: `functions.${name}.file`, message: fileError }] : []),
          ]}
          dirty={dirty}
        />
      )}
