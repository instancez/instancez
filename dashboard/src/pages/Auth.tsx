...
// ref in YAML, value in .env, rendered first); the client ID is an ordinary
// setting stored as a literal YAML value (a hand-edited ${VAR} ref is respected
// and rendered as env-managed). The redirect URL is not user-editable. It is
// fixed by the app's own URL and shown read-only for the user to copy into the
// provider console (see oauth_callback_base).
...
export function AuthPage() {
  ...
  const { config, save, saving, saveErrors, dotenvWritable, oauthCallbackBase } = useConfig();
                    ...
                    <VStack gap="4" align="stretch">
                      ...
                      {isEnabled && providerConfig && (
                        <VStack gap="3" align="stretch">
                              ...
                              return (
                                <Box key={field.key} borderTopWidth="1px" borderColor="border">
                                  <VarRow
                                    ...
                                    tail={envVarStatus[refName]?.tail}
                                    canWrite={canWriteSecrets}
                                    showEnvName={showEnvName}
                          ...
                          <Field
                            label="Redirect URL"
                            hint={`Paste this into your ${provider === "google" ? "Google" : "GitHub"} OAuth app's authorized redirect URIs.`}
                          >
                            <HStack gap="2" w="full">
                              <Input
                                mono
                                ...
                                value={oauthCallbackBase ? oauthCallbackBase + provider : "…"}
                                onFocus={(e) => e.currentTarget.select()}
                              />
                              <CopyButton
                                value={oauthCallbackBase ? oauthCallbackBase + provider : ""}
                                label={`Copy ${provider} redirect URL`}
                              />
                            </HStack>
                          </Field>
                        </VStack>
