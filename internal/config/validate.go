...
func validateRLS(parentPath string, policies []domain.RLSPolicy) domain.ValidationErrors {
	...
	for i, p := range policies {
		...
		var hasSelectOrDelete, hasInsert, hasUpdate bool
		...
		// A policy entry is either one command, or all four sharing the same
		// expression(s) — no partial combos (e.g. [insert, update] alone).
		// Matches the dashboard's operations picker, which only offers a
		// single command or "ALL". Skip when there are no valid ops at all —
		// the empty/all-invalid cases already have their own error above.
		if len(distinctOps) > 0 && len(distinctOps) != 1 && len(distinctOps) != len(validRLSOps) {
			errs = append(errs, &domain.ValidationError{
				Path:       ppath + ".operations",
				Message:    "operations must be exactly one command, or all four (select, insert, update, delete)",
				Suggestion: "Split into one rls entry per operation, or list all four together",
			})
		}
		...
		needsUsing := hasSelectOrDelete || hasUpdate
		needsWithCheck := hasInsert || hasUpdate
		hasUsing := p.Using != ""
		hasWithCheck := p.WithCheck != ""

		// Postgres auto-fills the missing clause from the other one for an
		// UPDATE policy, but the two directions are NOT symmetric:
		//
		//   - using-only: Postgres reuses `using` as `with_check` too,
		//     checked against the new row. This holds for both permissive
		//     and restrictive policies.
		//   - with_check-only: Postgres does NOT fall back to `with_check`
		//     (or default to `true`) for the missing `using`. For a
		//     restrictive policy this is harmless: a missing `using` there
		//     is a no-op, since restrictive policies AND together and add
		//     no visibility grant of their own. For a permissive policy
		//     (including the default empty type), a missing `using` makes
		//     the policy grant zero row visibility for update, so no row is
		//     ever updatable through it regardless of `with_check`. Verified
		//     empirically against Postgres 16, not just documented
		//     behavior. So the with_check-only auto-fill only counts as
		//     satisfying `using` for restrictive policies.
		usingSatisfiedByAutoFill := hasUpdate && !hasSelectOrDelete && hasWithCheck && p.Type == "restrictive"
		withCheckSatisfiedByAutoFill := hasUpdate && !hasInsert && hasUsing

		errs = append(errs, validateRLSExpr(ppath+".using", p.Using)...)
		errs = append(errs, validateRLSExpr(ppath+".with_check", p.WithCheck)...)

		if hasUsing && !needsUsing {
			errs = append(errs, &domain.ValidationError{
				Path:       ppath + ".using",
				Message:    "using has no effect: none of the listed operations read it (select, update, delete)",
				Suggestion: "Remove using, or add select/update/delete to operations",
			})
		}
		if hasWithCheck && !needsWithCheck {
			errs = append(errs, &domain.ValidationError{
				Path:       ppath + ".with_check",
				Message:    "with_check has no effect: none of the listed operations read it (insert, update)",
				Suggestion: "Remove with_check, or add insert/update to operations",
			})
		}
		if needsUsing && !hasUsing && !usingSatisfiedByAutoFill {
			errs = append(errs, &domain.ValidationError{
				Path:    ppath + ".using",
				Message: "using is required: select, delete, or update reads it and has no fallback",
			})
		}
		if needsWithCheck && !hasWithCheck && !withCheckSatisfiedByAutoFill {
			errs = append(errs, &domain.ValidationError{
				Path:    ppath + ".with_check",
				Message: "with_check is required: insert or update reads it and has no fallback",
			})
		}
	}
	return errs
...
// validateRLSExpr applies the DDL-injection guards (bare `;`, pasted CREATE
// statements) that used to run once against the shared `check` field, now
// run independently against `using` and `with_check`.
func validateRLSExpr(path, expr string) domain.ValidationErrors {
	var errs domain.ValidationErrors
	switch {
	case expr == "":
		// Emptiness is a required/not-applicable concern, handled by the caller.
	case strings.Contains(expr, ";"):
		errs = append(errs, &domain.ValidationError{
			Path:       path,
			Message:    "must be a single boolean expression without `;` — it is inserted into the generated CREATE POLICY statement",
			Suggestion: "Example: user_id = auth.uid()",
		})
	case rlsCheckDDLRE.MatchString(expr):
		errs = append(errs, &domain.ValidationError{
			Path:       path,
			Message:    "must be a boolean expression, not a CREATE statement — instancez generates the policy DDL for you",
			Suggestion: "Keep only the expression, e.g. user_id = auth.uid()",
		})
	}
	return errs
}
