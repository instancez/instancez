...
func TestValidate_RLSMissingUsing(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"select"}, Using: ""}}
	cfg.Tables["todos"] = table

	errs := Validate(cfg)
	assertHasErrorAt(t, errs, "tables.todos.rls[0].using")
}

func TestValidate_RLSInvalidOp(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"truncate"}, Using: "true"}}
	cfg.Tables["todos"] = table
...
func TestValidate_RLSWithCheckUnusedOnSelectOnly(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"select"}, Using: "true", WithCheck: "true"}}
	cfg.Tables["todos"] = table

	errs := Validate(cfg)
	assertHasErrorAt(t, errs, "tables.todos.rls[0].with_check")
}

func TestValidate_RLSUsingUnusedOnInsertOnly(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"insert"}, Using: "true", WithCheck: "true"}}
	cfg.Tables["todos"] = table
...
func TestValidate_RLSMissingWithCheckOnInsert(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"insert"}, Using: "true"}}
	cfg.Tables["todos"] = table
...
func TestValidate_RLSUpdateOnlyAutoFillFromUsing(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"update"}, Using: "user_id = auth.uid()"}}
	cfg.Tables["todos"] = table

	errs := Validate(cfg)
	for _, e := range errs {
		if strings.HasPrefix(e.Path, "tables.todos.rls[0]") {
			t.Fatalf("update-only policy with only `using` set should be valid (Postgres auto-fills with_check), got %s: %s", e.Path, e.Message)
		}
	}
}

func TestValidate_RLSUpdateOnlyWithCheckOnlyPermissiveRequiresUsing(t *testing.T) {
	// Permissive (the default type) update-only policy with only `with_check`
	// set: Postgres does NOT fall back to `with_check` (or `true`) for the
	// missing `using` here. The policy ends up granting zero row visibility
	// for update, so no row is ever updatable through it — confirmed
	// empirically against Postgres 16. `using` is required.
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"update"}, WithCheck: "user_id = auth.uid()"}}
	cfg.Tables["todos"] = table
...
func TestValidate_RLSUpdateOnlyWithCheckRestrictiveIsValid(t *testing.T) {
	// Same shape, but restrictive: a missing `using` is a no-op for a
	// restrictive policy (restrictive policies AND together and grant no
	// visibility of their own), so `with_check` alone is a legitimate
	// write-gate on top of visibility granted elsewhere.
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"update"}, WithCheck: "user_id = auth.uid()", Type: "restrictive"}}
	cfg.Tables["todos"] = table

	errs := Validate(cfg)
	for _, e := range errs {
		if strings.HasPrefix(e.Path, "tables.todos.rls[0]") {
			t.Fatalf("restrictive update-only policy with only `with_check` set should be valid, got %s: %s", e.Path, e.Message)
		}
	}
}

func TestValidate_RLSUpdatePlusSelectRequiresUsing(t *testing.T) {
	// Unlike update-only, once select/delete shares the entry, using has no
	// fallback — the auto-fill exception only rescues update-only entries.
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"select", "update"}, WithCheck: "user_id = auth.uid()"}}
	cfg.Tables["todos"] = table
...
func TestValidate_RLSTwoOpComboRejected(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"insert", "update"}, Using: "true", WithCheck: "true"}}
	cfg.Tables["todos"] = table
...
func TestValidate_RLSThreeOpComboRejected(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"insert", "update", "delete"}, Using: "true", WithCheck: "true"}}
	cfg.Tables["todos"] = table
...
func TestValidate_RLSAllFourOpsAccepted(t *testing.T) {
	cfg := validBaseConfig()
	table := cfg.Tables["todos"]
	table.RLS = []domain.RLSPolicy{{Operations: []string{"select", "insert", "update", "delete"}, Using: "true", WithCheck: "true"}}
	cfg.Tables["todos"] = table

	errs := Validate(cfg)
	for _, e := range errs {
		if strings.HasPrefix(e.Path, "tables.todos.rls[0]") {
			t.Fatalf("all-four-ops policy should be valid, got %s: %s", e.Path, e.Message)
		}
	}
}
...
// TestValidate_OAuthRedirectURLOptional pins that an oauth provider block
// without redirect_url passes validation. The auth handler fills a blank one
// with this server's own callback URL at request time (see
// AuthHandler.effectiveOAuthConfig), so requiring it in config would force
// users to hand-type a value the platform already knows. client_id and
// client_secret stay required.
func TestValidate_OAuthRedirectURLOptional(t *testing.T) {
	cfg := validBaseConfig()
	...
	errs := Validate(cfg)
	for _, e := range errs {
		if e.Path == "auth.oauth.google.redirect_url" {
			t.Fatalf("redirect_url should be optional, got error: %s", e.Message)
		}
	}
}
		...
		Storage: map[string]domain.Bucket{
			"avatars": {
				...
				RLS: []domain.RLSPolicy{
					...
					{Operations: []string{"delete"}, Using: "uploaded_by = auth.uid()"},
				},
			},
		},
	}
...
// TestValidate_RLSExpressionShape pins the RLS expression contract: a boolean
// expression interpolated into CREATE POLICY ... USING (...) / WITH CHECK (...).
// Statement separators and pasted DDL are rejected — a stray `;` would
// otherwise be spliced into the generated DDL verbatim.
func TestValidate_RLSExpressionShape(t *testing.T) {
	withUsing := func(expr string) *domain.Config {
		cfg := validBaseConfig()
		table := cfg.Tables["todos"]
		table.RLS = []domain.RLSPolicy{
			{Operations: []string{"select"}, Using: expr},
		}
		cfg.Tables["todos"] = table
		return cfg
	}
	...
	for _, good := range []string{
		...
		for _, e := range Validate(withUsing(good)) {
			if strings.HasPrefix(e.Path, "tables.todos.rls[0].using") {
				t.Fatalf("expression %q should be valid, got %s: %s", good, e.Path, e.Message)
			}
		}
	}
}
