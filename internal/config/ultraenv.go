...
const ultraEnvPrefix = "ULTRA_ENV_"
...
// This is the single shared parser used by both loadDotenv and LoadUltraEnv.
...
// LoadUltraEnv builds the in-memory ULTRA_ENV_ namespace from:
//  1. <dir>/.env      — base file, loaded first (missing = not an error)
//  2. <dir>/.<mode>.env — mode-specific overlay (missing = not an error)
//  3. ULTRA_ENV_* keys from os.Environ() — highest precedence
//
// Only keys with the "ULTRA_ENV_" prefix are included in the returned map.
...
func LoadUltraEnv(dir, mode string) (map[string]string, error) {
	...
	// Helper: parse a dotenv file and overlay ULTRA_ENV_* keys.
	overlay := func(path string) error {
		...
		for k, v := range parsed {
			if strings.HasPrefix(k, ultraEnvPrefix) {
				result[k] = v
			}
		}
		return nil
