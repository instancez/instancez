...
// role-switching wrapper (exposing the NOINHERIT behaviour: the authenticator
// login carries no table privileges until SET LOCAL ROLE is issued inside a
// transaction) plus the superuser connection string. Creating a BYPASSRLS role
// requires superuser on Postgres < 16 — the same way production provisions
// roles from a superuser DSN — so tests that need one dial the superURL rather
// than the CREATEROLE owner login.
func StartContainerWithRawAuth(t *testing.T, image ...string) (domain.OwnerDB, domain.RequestDB, *postgres.DB, string) {
	t.Helper()
	img := PostgresImage()
	if len(image) > 0 {
		img = image[0]
	...
	return owner, auth, rawAuth, superURL
}
