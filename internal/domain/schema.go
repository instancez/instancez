...
// ProjectCloud links a project to instancez Cloud. The field is read by the
// `cloud` package (deploy, status) via a standalone YAML walk; it is declared
...
// CORS carries the only knob apps actually need — the same shape Supabase's
// own gateway exposes. Methods/headers/credentials/max_age are fixed platform
// defaults (see corsMiddleware) rather than per-app config.
type CORS struct {
...
// two is NOT symmetric: a using-only policy has Using reused as WithCheck too
// (true for both permissive and restrictive policies), but a with_check-only
// policy only gets Using auto-filled when Type is "restrictive" (there, a
// missing Using is a no-op). A with_check-only policy of the default
// permissive type instead grants zero row visibility for update; Using must
