...
func TestDriftHeartbeatLogsPeriodically(t *testing.T) {
	...
	var logged int32
	go runDriftHeartbeat(ctx, func() *DriftTracker { return tracker }, logger, 20*time.Millisecond, func() {
...
// goroutine on its very next tick.
func TestDriftHeartbeatFollowsTrackerSwap(t *testing.T) {
	db := newFakeDB(t)
	roles := domain.DefaultRoles()
	authDB := newFakeRequestDB(t)
	...
	engine := NewEngine(cfg, domain.OwnerDB{Database: db}, authDB, roles, WithMode(ModeProd), WithLogger(logger))

	okTracker := NewDriftTracker("test://boot")
	okTracker.MarkOK("boot", time.Now())
	engine.drift = okTracker

	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	tickCh := make(chan struct{}, 4)
	go runDriftHeartbeat(ctx, engine.Drift, logger, 20*time.Millisecond, func() {
		select {
		case tickCh <- struct{}{}:
		default:
		}
	...
	// Boot tracker is OK, so no ticks should fire yet.
	select {
	case <-tickCh:
		t.Fatalf("heartbeat fired while boot tracker was OK")
	case <-time.After(100 * time.Millisecond):
	}

	// Simulate runWatcher's post-reload swap (engine.go: e.drift = tracker).
	driftedTracker := NewDriftTracker("test://reload")
	driftedTracker.MarkOK("boot", time.Now())
	driftedTracker.MarkDrift("bad", "boom", time.Now())
	engine.mu.Lock()
	engine.drift = driftedTracker
	engine.mu.Unlock()

	select {
	case <-tickCh:
	case <-time.After(500 * time.Millisecond):
		t.Fatalf("heartbeat did not observe the swapped tracker's drift state")
	}
}
