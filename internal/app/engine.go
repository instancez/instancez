...
func (e *Engine) Start(ctx context.Context) error {
	...
	// each tick rather than capturing e.drift here, since runWatcher swaps
	// e.drift on every successful reload.
	if e.drift != nil {
		go runDriftHeartbeat(ctx, e.Drift, e.logger, 10*time.Minute, nil)
	}

	// Seed the JWT signing key now that migrations have created auth.jwt_keys,
	// so /.well-known/jwks.json serves a public key before the first login.
	// Active() mints the RS256 key on first use and is a no-op once a row
	// exists. This has to run after the migrate step above, or the INSERT hits a
	// missing table on a fresh app. Best-effort: a failure is retried on first
	// token issuance, so it never blocks startup.
	if _, err := NewJWTKeyManager(e.ownerDB).Active(ctx); err != nil {
		e.logger.Warn("jwt: could not seed signing key at boot", "error", err)
	}
...
func (e *Engine) waitForShutdown(ctx context.Context, srvErrCh <-chan error) error {
	...
	case err := <-srvErrCh:
		// Shut down on the way out, but the server error is what the caller
		// needs to see — a shutdown error here would only mask it.
		_ = e.shutdown()
		return err
	}
...
// called on each tick rather than the tracker being captured once, since the
// engine reassigns its drift tracker on every successful watcher reload.
// Returns when ctx is cancelled. The onTick callback is for tests; nil in
// production.
func runDriftHeartbeat(ctx context.Context, getTracker func() *DriftTracker, logger *slog.Logger, interval time.Duration, onTick func()) {
	if interval <= 0 {
