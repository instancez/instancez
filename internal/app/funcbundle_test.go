...
func TestFetchAndExtractRegularFilesDirsAndSymlink(t *testing.T) {
	...
	dest := t.TempDir()
	dir, version, err := FetchAndExtract(context.Background(), path+"#v123", dest)
	if err != nil {
...
func TestFetchAndExtractDirNameMatchesVersion(t *testing.T) {
	...
	dir, _, err := FetchAndExtract(context.Background(), path+"#abcdef", dest)
	if err != nil {
		t.Fatalf("FetchAndExtract: %v", err)
	}
	...
	dir2, _, err := FetchAndExtract(context.Background(), path+"#abcdef", dest)
	if err != nil {
...
func TestFetchAndExtractRejectsPathTraversal(t *testing.T) {
	...
	_, _, err := FetchAndExtract(context.Background(), path+"#v1", dest)
	if err == nil {
...
func TestFetchAndExtractNoVersionUsesContentHash(t *testing.T) {
	...
	dir, version, err := FetchAndExtract(context.Background(), path, dest)
	if err != nil {
		t.Fatalf("FetchAndExtract: %v", err)
	}
	if version != "" {
