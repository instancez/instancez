...
func TestClientSendsExtraHeadersFromEnv(t *testing.T) {
	t.Setenv("INSTANCEZ_CLOUD_HEADERS", "X-Edge-Bypass: s3cr3t\nX-Other: two")
	var gotBypass, gotOther string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotBypass = r.Header.Get("X-Edge-Bypass")
	...
	assert.Equal(t, "s3cr3t", gotBypass, "extra header from INSTANCEZ_CLOUD_HEADERS must be sent")
	assert.Equal(t, "two", gotOther, "multiple newline-separated headers must all be sent")
}

func TestExtraHeadersUnsetIsNil(t *testing.T) {
	t.Setenv("INSTANCEZ_CLOUD_HEADERS", "")
...
func TestClientUploadYAML(t *testing.T) {
	...
	c := NewClient(srv.URL, "instancez_pat_test")
	dropped, diff, err := c.UploadYAML("app-uuid", "version: 1\n")
	assert.NoError(t, err)
...
func TestClientUploadYAMLValidationFailed(t *testing.T) {
	...
	c := NewClient(srv.URL, "instancez_pat_test")
	_, _, err := c.UploadYAML("app-uuid", "version: 1\n")
...
func TestUploadSecrets(t *testing.T) {
	...
	c := NewClient(srv.URL, "pat")
	if err := c.UploadSecrets("proj1", map[string]string{"INSTANCEZ_ENV_FOO": "bar"}); err != nil {
...
func TestClientPreviewConfig(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		assert.Equal(t, "POST", r.Method)
	...
	c := NewClient(srv.URL, "instancez_pat_test")
	dropped, diff, err := c.PreviewConfig("app-uuid", "version: 1\n")
	require.NoError(t, err)
	assert.Empty(t, dropped)
	require.True(t, diff.HasChanges)
...
func TestClientGetApp(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(map[string]any{
			...
			"status":          "DEPLOYED",
			"publishable_key": "inz_publishable_abc",
	...
	assert.Equal(t, "inz_publishable_abc", resp.PublishableKey)
