...
// maxTimeout caps a function's per-request timeout. It sits below the app
// Lambda's 30s execution limit, so a long-running function hits its own
// deadline and returns a clean error before AWS hard-kills the whole
// invocation. A configured timeout above this is clamped down silently.
const maxTimeout = 25 * time.Second
...
type Options struct {
	...
	LoopbackURL string
	// PublishableKey is the opaque publishable API key the data-access clients
	// send as the `apikey` header (and as the Bearer token for anonymous calls).
	// Empty when a deployment configures no key; ctx.supabase then sends none.
	PublishableKey string
	// SecretKey is the opaque secret API key ctx.serviceClient sends as its
	// `apikey` header to run as service_role. Empty disables escalation:
	// serviceClient falls back to the publishable key, i.e. no bypass.
	SecretKey string
	...
	// HealthTimeout is how long a freshly spawned worker has to answer /healthz
	// before it is considered dead. When <= 0 a default of 30s is used. The full
	// duration is only ever spent on a worker that never comes up; a healthy one
	// returns as soon as it responds. The default is generous on purpose: on a
	// Lambda cold start the worker loads the shim and vendored deps off cold /tmp,
	// which can take several seconds — a tight deadline there fails the whole
	// engine boot and crash-loops the function.
...
func (r *Runtime) Invoke(ctx context.Context, in domain.FunctionRequest) (*domain.FunctionResponse, error) {
	...
	ctxJSON, err := json.Marshal(map[string]any{
		...
		"dataPlane": map[string]any{
			...
			"publishableKey": r.opts.PublishableKey,
