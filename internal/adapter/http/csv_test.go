...
func TestCoerceCSVValue(t *testing.T) {
	tests := []struct {
		...
		// Arrays: the element-parsing switch never matches a "{...}" literal,
		// so array values pass through unchanged as text. Postgres parses the
		// literal itself when the value reaches the array column, the same
		// as the numeric/decimal case above.
		{"{1,2,3}", "integer[]", "{1,2,3}", false},
		{"{a,b,c}", "text[]", "{a,b,c}", false},
		{"", "integer[]", nil, true},
		{"", "text[]", "", false},
	}
