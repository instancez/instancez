...
type AuthHandler struct {
	...
	db      domain.Database    // only for the group-level service-role session; handlers use authSvc
}
...
// Mount registers the /auth/v1/* routes on the root router group. Every route
...
func (h *AuthHandler) Mount(root *gin.RouterGroup) {
	...
	auth.POST("/signup", apiKeyGuard(h.jwtKeys), h.handleSignupDispatch)
	auth.POST("/token", apiKeyGuard(h.jwtKeys), h.handleToken)
	auth.GET("/user", apiKeyGuard(h.jwtKeys), jwtAuth(h.jwtKeys, true), h.handleGetUser)
	auth.PUT("/user", apiKeyGuard(h.jwtKeys), jwtAuth(h.jwtKeys, true), h.handleUpdateUser)
	auth.POST("/logout", apiKeyGuard(h.jwtKeys), jwtAuth(h.jwtKeys, true), h.handleLogout)
	auth.GET("/settings", apiKeyGuard(h.jwtKeys), h.handleSettings)

	if h.cfg.Auth.Email != nil {
		auth.POST("/recover", apiKeyGuard(h.jwtKeys), h.handleRecover)
		auth.POST("/verify", apiKeyGuard(h.jwtKeys), h.handleVerify)
		// GET /verify is the URL embedded in emailed magic-link/recovery
		// links — clicked directly in a browser, never fetched by
		// supabase-js. No apikey.
		auth.GET("/verify", h.handleVerifyGET)
		auth.POST("/otp", apiKeyGuard(h.jwtKeys), h.handleOTP)
		auth.POST("/resend", apiKeyGuard(h.jwtKeys), h.handleResend)
		// supabase-js calls reauthenticate() over GET (matching GoTrue); keep
		// POST too so existing direct callers don't break.
		auth.GET("/reauthenticate", apiKeyGuard(h.jwtKeys), jwtAuth(h.jwtKeys, true), h.handleReauthenticate)
		auth.POST("/reauthenticate", apiKeyGuard(h.jwtKeys), jwtAuth(h.jwtKeys, true), h.handleReauthenticate)
	}
	auth.POST("/token/verify", apiKeyGuard(h.jwtKeys), h.handleTokenVerify)
	auth.GET("/.well-known/jwks.json", apiKeyGuard(h.jwtKeys), h.handleJWKS)
	...
	admin := auth.Group("/admin", apiKeyGuard(h.jwtKeys), adminKeyAuth())
	...
	auth.POST("/invite", apiKeyGuard(h.jwtKeys), adminKeyAuth(), h.handleAdminInvite)

	h.MountMFA(auth)

	// /authorize is handed to a browser/webview to navigate to, and
	// /callback/<provider> is the provider's redirect target — plain browser
	// navigation in both cases, never fetched by supabase-js. No apikey.
...
func (h *AuthHandler) handleAuthorize(c *gin.Context) {
	...
	cfg = h.effectiveOAuthConfig(provider, cfg)
...
// oauthCallbackFail reports an OAuth callback failure. The callback lands in the
// browser, so when a valid redirect target is known it sends the browser there
// with GoTrue-style error params — query string for PKCE, fragment for implicit,
// matching the success path — which supabase-js's detectSessionInUrl surfaces to
// the app. With no usable redirect it falls back to JSON. The description is kept
// generic on purpose; the underlying cause (e.g. a bad client secret) stays in
// the logs rather than leaking to the browser.
func (h *AuthHandler) oauthCallbackFail(c *gin.Context, redirectTo string, isPKCE bool, desc string) {
	if target, ok := h.oauthLanding(redirectTo); ok {
		p := url.Values{}
		p.Set("error", "server_error")
		p.Set("error_code", "unexpected_failure")
		p.Set("error_description", desc)
		c.Redirect(http.StatusFound, appendOAuthParams(target, isPKCE, p))
		return
	}
	problemJSON(c, 500, "oauth_error", desc)
}

// oauthLanding picks where to send the browser after the OAuth callback. It
// prefers a caller-supplied redirect_to when it clears the allowlist, otherwise
// the first configured redirect_url — instancez's closest equivalent to
// Supabase's SITE_URL default. Returns ("", false) when neither is available,
// which leaves the caller to fall back to a JSON response.
func (h *AuthHandler) oauthLanding(redirectTo string) (string, bool) {
	if redirectTo != "" && h.redirectAllowed(redirectTo) {
		return redirectTo, true
	}
	if urls := h.cfg.Auth.RedirectURLs; len(urls) > 0 {
		return urls[0], true
	}
	return "", false
}

// appendOAuthParams attaches OAuth result params to a landing URL: as a query
// string for PKCE (matching the ?code= success channel), as a fragment
// otherwise (matching #access_token=). supabase-js reads either.
func appendOAuthParams(target string, isPKCE bool, v url.Values) string {
	if isPKCE {
		sep := "?"
...
func (h *AuthHandler) handleOAuthCallback(provider string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if target, ok := h.oauthLanding(redirectTo); ok {
			...
			c.Redirect(http.StatusFound, target+"#"+frag.Encode())
			return
		}
		// Only when no redirect_to was passed AND no redirect_urls are configured
		// is there nowhere to land the browser — then the session is returned as
		// raw JSON instead of a redirect.
		c.JSON(200, session)
	}
}
...
// serverBaseURL is the externally reachable origin of this app:
// INSTANCEZ_BASE_URL when set (the deployer points it at the app's public
// frontend URL, apex custom domains already rewritten to www.), else a
// localhost fallback for local dev. Both the auth handler and the admin status
// endpoint derive external links from this one source so they can't disagree.
func serverBaseURL(cfg *domain.Config) string {
...
// the shared h.cfg is never mutated.
func (h *AuthHandler) effectiveOAuthConfig(provider string, cfg *domain.OAuthProvider) *domain.OAuthProvider {
	if cfg == nil || cfg.RedirectURL != "" {
		return cfg
	}
	eff := *cfg
	eff.RedirectURL = oauthCallbackBase(h.cfg) + provider
	return &eff
}
