...
func newMFAHarness(t *testing.T, svc *stubAuthService) *mfaHarness {
	...
	t.Setenv("INSTANCEZ_PUBLISHABLE_KEY", "inz_publishable_mfatest")
	km := stubKeys(t)
	h := &AuthHandler{
...
// TestMFA_VerifyRejectsChallengeAtAttemptCap asserts the handler maps
// ErrChallengeTooManyAttempts to 429 without ever comparing the TOTP code.
func TestMFA_VerifyRejectsChallengeAtAttemptCap(t *testing.T) {
	svc := &stubAuthService{
		getFactorForVerifyFn: func(ctx context.Context, fID, userID string) (domain.MFAFactor, error) {
			return domain.MFAFactor{Secret: "JBSWY3DPEHPK3PXP", Status: "verified"}, nil
		},
	...
	w := m.do("POST", "/auth/v1/factors/any/verify", `{"challenge_id":"c1","code":"000000"}`)
	if w.Code != 429 {
		t.Fatalf("expected 429 at the attempt cap, got %d: %s", w.Code, w.Body.String())
	}
}
