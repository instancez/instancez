...
// handleKeys returns the project's API keys: the publishable key (client-safe,
// runs as anon) and the secret key (server-side, runs as service_role). Both
// are the opaque env-configured values the dashboard presents for copying,
// mirroring Supabase's Settings → API keys. The route is already gated by
// adminKeyAuth, so returning the secret key here is intended.
func (h *AdminHandler) handleKeys(c *gin.Context) {
	c.JSON(200, gin.H{
		"publishable_key": os.Getenv("INSTANCEZ_PUBLISHABLE_KEY"),
		"secret_key":      os.Getenv("INSTANCEZ_SECRET_KEY"),
	})
}
...
// envVarStatus reports whether name is set in the server process and, when it
// is, the last-4-char tail of its value so the dashboard can show a masked
// confirmation of which value landed. The plaintext is never returned.
func envVarStatus(name string) gin.H {
	val, set := os.LookupEnv(name)
	if !set {
		return gin.H{"set": false}
	}
	return gin.H{"set": true, "tail": tailOf(val)}
}

// tailOf returns the last four characters of v (or the whole value when
// shorter). Mirrors the platform's tailOf in the app_secrets store.
func tailOf(v string) string {
	r := []rune(v)
	if len(r) <= 4 {
		return v
	}
...
// process, and a last-4 tail for the set ones. It covers every ${VAR}
// reference in the current raw config source plus any names the client asks
// about via ?names=A,B (the dashboard masks provider vars before their refs
// are saved into the YAML). Values are never returned, only the tail.
...
func containsDestructive(stmt string) bool {
	...
	// No migration plan ever emits a literal "DELETE FROM" — every occurrence
	// of "DELETE " in a plan comes from the baseline
	// "GRANT ... DELETE ON TABLES ..." privilege grant, so matching on it only
	// produced false positives (is_destructive=true on every fresh deploy).
	return len(upper) > 0 && (contains(upper, "DROP ") || contains(upper, "TRUNCATE "))
}
