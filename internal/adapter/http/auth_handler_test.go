...
func TestEffectiveOAuthConfig(t *testing.T) {
	...
	explicit := &domain.OAuthProvider{ClientID: "id", ClientSecret: "s", RedirectURL: "https://custom.example/cb"}
	if out := h.effectiveOAuthConfig("google", explicit); out.RedirectURL != "https://custom.example/cb" {
		t.Fatalf("explicit redirect_url must be untouched, got %q", out.RedirectURL)
	}
}
...
// TestJWTAuth_AnonAllowedPath pins the PostgREST-compatible behavior on an
// anon-allowed route (required=false): a missing bearer runs as anon, a
// present-but-bad bearer is a 401 rather than a silent anon downgrade, exp
// gets a 30s skew allowance, and a token with no role claim resolves to anon.
func TestJWTAuth_AnonAllowedPath(t *testing.T) {
	gin.SetMode(gin.TestMode)
	km := stubKeys(t)

	uid := "11111111-2222-3333-4444-555555555555"
	expiredBeyondSkew := signToken(t, km, jwt.MapClaims{
		"sub": uid, "role": "authenticated",
		"iat": time.Now().Add(-time.Hour).Unix(),
		"exp": time.Now().Add(-5 * time.Minute).Unix(),
	})
	expiredWithinSkew := signToken(t, km, jwt.MapClaims{
		"sub": uid, "role": "authenticated",
		"iat": time.Now().Add(-time.Hour).Unix(),
		"exp": time.Now().Add(-10 * time.Second).Unix(),
	})
	noRoleClaim := signToken(t, km, jwt.MapClaims{
		"sub": uid,
		"iat": time.Now().Unix(),
		"exp": time.Now().Add(time.Hour).Unix(),
	})
	// A signature-valid token that claims service_role must NOT be honored:
	// service_role is only reachable via the secret key, never a bearer.
	serviceRoleClaim := signToken(t, km, jwt.MapClaims{
		"sub": uid, "role": "service_role",
		"iat": time.Now().Unix(),
		"exp": time.Now().Add(time.Hour).Unix(),
	})
	...
	cases := []struct {
		...
		bearer     string
		wantStatus int
		wantRole   string // only checked on 200
	}{
		{"no bearer runs as anon", "", 200, "anon"},
		{"garbage bearer is 401", "Bearer not-a-jwt", 401, ""},
		{"expired beyond skew is 401", "Bearer " + expiredBeyondSkew, 401, ""},
		{"expired within 30s skew accepted", "Bearer " + expiredWithinSkew, 200, "authenticated"},
		{"valid token without role claim is anon", "Bearer " + noRoleClaim, 200, "anon"},
		{"valid token claiming service_role is 401", "Bearer " + serviceRoleClaim, 401, ""},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			r := gin.New()
			...
			if tc.wantStatus == 200 {
				...
				if body["role"] != tc.wantRole {
					t.Errorf("role = %v, want %q", body["role"], tc.wantRole)
				}
			}
		})
	}
...
func TestSignupGating_AdminCreateUser_IgnoresAllowSignup(t *testing.T) {
	gin.SetMode(gin.TestMode)
	t.Setenv("INSTANCEZ_SECRET_KEY", "test-admin-key")
	...
	req.Header.Set("apikey", "test-admin-key")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
...
// failingOAuthProvider stands in for a misconfigured provider (e.g. a bad client
// secret): its ExchangeCode always errors, mirroring the 401 invalid_client that
// Google returns.
type failingOAuthProvider struct{}
...
// TestHandleOAuthCallback_ExchangeFailure verifies that a failed code exchange
// (e.g. bad client secret) sends the browser back to the app with GoTrue-style
// error params rather than dumping a raw 502 JSON body, and falls back to a 500
// only when no valid redirect target is known.
func TestHandleOAuthCallback_ExchangeFailure(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	// Allowed redirect → 302 back to the app with error params in the fragment.
	w := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(w)
	...
	if !strings.HasPrefix(loc, "https://app.example.com/cb#") ||
		!strings.Contains(loc, "error=server_error") ||
		!strings.Contains(loc, "error_description=") {
		t.Errorf("with redirect_to: Location = %q", loc)
	}

	// No redirect_to but redirect_urls configured → land on the first
	// redirect_url (SITE_URL-style default), still 302 with error params.
	w = httptest.NewRecorder()
	c, _ = gin.CreateTestContext(w)
	c.Request = httptest.NewRequest("GET", "/auth/v1/callback/failcb?state=s1&code=abc", nil)
	c.Request.AddCookie(&http.Cookie{Name: "oauth_state", Value: "s1"})
	...
	if loc := w.Header().Get("Location"); !strings.HasPrefix(loc, "https://app.example.com#error=server_error") {
		t.Errorf("no redirect_to (with redirect_urls): Location = %q", loc)
	}

	// No redirect_to and no redirect_urls at all → nowhere to land → 500 JSON.
