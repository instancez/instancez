...
func runLogin(force bool) error {
	...
	if !force {
		if existing, err := cloud.Load(); err == nil && existing.PAT != "" {
			who := existing.Email
			if who == "" {
				// Email is never persisted alongside the PAT (see cloud.Credentials);
				// fetch it live the same way `inz cloud whoami` does.
				if resp, err := cloud.NewClient(cloud.APIURL(), existing.PAT).Whoami(); err == nil {
					who = resp.Email
				}
			}
			if who == "" {
				fmt.Println("Already logged in. Use --force to re-authenticate.")
			} else {
...
// the single source of truth for the flow — both `inz cloud login` and the inline
...
func runDeviceCodeFlow() (cloud.Credentials, error) {
	...
	if err != nil {
		...
		case errors.Is(err, cloud.ErrDeviceExpired):
			return cloud.Credentials{}, errors.New("code expired before confirmation; run `inz cloud login` again")
		default:
...
// returns a hard error pointing at `inz cloud login` rather than hanging on a
...
func ensureLoggedIn(opts ensureLoginOpts) (cloud.Credentials, error) {
	...
	if !opts.isTTY() {
		return cloud.Credentials{}, errors.New(
			"not logged in; run `inz cloud login` first (cannot prompt in a non-interactive session)")
	}
	...
	if !opts.assumeYes {
		if !opts.confirm("Sign in now? [Y/n] ") {
			return cloud.Credentials{}, errors.New("login required; run `inz cloud login` to sign in")
		}
	}
	return opts.runFlow()
}
