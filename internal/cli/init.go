...
func runInit(opts initOptions) error {
	...
	// Code functions run in Node workers, and `inz dev` refuses to boot when
	// functions are declared but node is missing. Rather than scaffolding a
	// project that fails its first `inz dev`, skip the example function on
	// node-less machines and say so loudly below.
	nodeOK := funcs.RequireNode() == nil
	...
	if yamlExists && !opts.force {
		...
		if err := applyWrite(dir, "instancez.yaml", func(_ string) (string, writeAction) {
			return scaffoldYAML(name, nodeOK), actionCreate
		}); err != nil {
			return err
		}
	}
	...
	// re-run. `inz dev` runs `npm ci` here on boot. Skipped entirely when node
	// is missing, matching the yaml scaffold above.
	if nodeOK {
		...
		alertf("  ! Node.js not found on PATH. Skipped the example code function.")
		alertf("    Code functions (the functions: block in instancez.yaml) need Node.js >= %d.", funcs.MinNodeMajorVersion)
		alertf("    Install it from https://nodejs.org to use them.")
	}
...
// scaffoldYAML renders the starter instancez.yaml. withFunctions gates the
// example code function: init omits it on machines without Node.js so the
// scaffold's first `inz dev` boots instead of failing the node precheck.
func scaffoldYAML(name string, withFunctions bool) string {
...
// alertf prints a warning line to stderr, in red when stderr is a terminal
// (and NO_COLOR is unset) so it stands out from the scaffold file listing.
func alertf(format string, a ...any) {
	msg := fmt.Sprintf(format, a...)
	if fi, err := os.Stderr.Stat(); err == nil && fi.Mode()&os.ModeCharDevice != 0 && os.Getenv("NO_COLOR") == "" {
...
#
# 'inz dev' also generates random INSTANCEZ_PUBLISHABLE_KEY and
# INSTANCEZ_SECRET_KEY into .development.env on first run (printed to the
# console; the secret key logs you into the dashboard). Set them here yourself to
# pin known values instead.
