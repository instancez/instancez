...
// requireServeAPIKeys enforces that both API keys are configured before serve
// boots. Flag values take precedence and are written into the env the rest of
// the process reads (middleware and the function runtime look them up there).
func requireServeAPIKeys(publishable, secret string) error {
	if publishable != "" {
		_ = os.Setenv("INSTANCEZ_PUBLISHABLE_KEY", publishable)
	}
	if secret != "" {
		_ = os.Setenv("INSTANCEZ_SECRET_KEY", secret)
	}
	var missing []string
	if os.Getenv("INSTANCEZ_PUBLISHABLE_KEY") == "" {
		missing = append(missing, "INSTANCEZ_PUBLISHABLE_KEY (or --publishable-key)")
	}
	if os.Getenv("INSTANCEZ_SECRET_KEY") == "" {
		missing = append(missing, "INSTANCEZ_SECRET_KEY (or --secret-key)")
	}
	if len(missing) > 0 {
		return fmt.Errorf("serve requires the API keys: set %s", strings.Join(missing, " and "))
	}
	// The two keys must differ. keyTier checks the secret first, so identical
	// values would make the client-shipped publishable key resolve to
	// service_role (BYPASSRLS) on every request — a total RLS bypass.
	if os.Getenv("INSTANCEZ_PUBLISHABLE_KEY") == os.Getenv("INSTANCEZ_SECRET_KEY") {
		return fmt.Errorf("INSTANCEZ_PUBLISHABLE_KEY and INSTANCEZ_SECRET_KEY must be different values")
	}
	return nil
}
...
func runServe(opts serveOptions) error {
	...
	// API keys are mandatory in production. Accept them from the
	// --publishable-key/--secret-key flags (which take precedence) or the
	// matching env vars (process env or a loaded .production.env).
	if err := requireServeAPIKeys(opts.publishableKey, opts.secretKey); err != nil {
		return err
	}
