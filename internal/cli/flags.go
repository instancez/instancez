...
func newServeFlagSet() *serveFlagSet {
	...
	fs.flags.StringVar(&fs.publishableKey, "publishable-key", "", "publishable API key (client-safe, anon role); required (env: INSTANCEZ_PUBLISHABLE_KEY)")
	fs.flags.StringVar(&fs.secretKey, "secret-key", "", "secret API key (service_role, admin surface); required (env: INSTANCEZ_SECRET_KEY)")
