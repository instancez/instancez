...
func TestParseServeFlagsAPIKeys(t *testing.T) {
	// Env fallback.
	got, err := parseServeFlags([]string{}, func(k string) string {
		return map[string]string{
			"INSTANCEZ_PUBLISHABLE_KEY": "inz_publishable_env",
			"INSTANCEZ_SECRET_KEY":      "inz_secret_env",
		}[k]
	})
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	if got.publishableKey != "inz_publishable_env" || got.secretKey != "inz_secret_env" {
		t.Fatalf("env keys not resolved: %+v", got)
	}

	// Explicit flags win over env.
	got, err = parseServeFlags(
		[]string{"--publishable-key", "inz_publishable_flag", "--secret-key", "inz_secret_flag"},
	...
	if got.publishableKey != "inz_publishable_flag" || got.secretKey != "inz_secret_flag" {
		t.Fatalf("flag keys should win over env: %+v", got)
	}
}

func TestRequireServeAPIKeys(t *testing.T) {
	t.Setenv("INSTANCEZ_PUBLISHABLE_KEY", "")
	t.Setenv("INSTANCEZ_SECRET_KEY", "")

	// Both missing → error naming both.
	err := requireServeAPIKeys("", "")
	if err == nil {
		t.Fatal("expected error when both keys missing")
	}
	if !strings.Contains(err.Error(), "INSTANCEZ_PUBLISHABLE_KEY") ||
		!strings.Contains(err.Error(), "INSTANCEZ_SECRET_KEY") {
		t.Fatalf("error should name both keys: %v", err)
	}

	// Flags satisfy the requirement and are exported to the env.
	if err := requireServeAPIKeys("inz_publishable_x", "inz_secret_x"); err != nil {
		t.Fatalf("unexpected error with both keys: %v", err)
	}
	if v := os.Getenv("INSTANCEZ_SECRET_KEY"); v != "inz_secret_x" {
		t.Fatalf("secret flag not exported to env, got %q", v)
	}

	// Identical keys are rejected: a shared value would make the publishable
	// key resolve to service_role (total RLS bypass).
	t.Setenv("INSTANCEZ_PUBLISHABLE_KEY", "")
	t.Setenv("INSTANCEZ_SECRET_KEY", "")
	if err := requireServeAPIKeys("same-value", "same-value"); err == nil {
		t.Fatal("expected error when publishable and secret keys are identical")
	}
}
